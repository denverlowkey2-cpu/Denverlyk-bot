    ...
    with LOCK:
        if uid not in USERS:
            USERS[uid] = {
                ...
                'history': [],
                'loss_streak': 0,
                'loss_streak_time': 0, # NEW: Track when streak started
                'banned': False
            }
        ...
        if USERS[uid]['loss_streak'] >= 3:
            if time.time() - USERS[uid].get('loss_streak_time', 0) > 3600: # 1 hour
        ...
        if USERS[uid]['expiry']:
            ...
            if datetime.now(timezone.utc) > exp_dt:
                ...
                USERS[uid]['loss_streak'] = 0 # Reset on expiry too
    return USERS[uid]
...
Expires in 5min | Signal ID: {sig['id'][-6:]}"""
...
def cmd_grant(message):
    ...
    try:
        ...
        with LOCK:
            ...
            u['loss_streak'] = 0 # Auto-reset on upgrade
                ...
                for sig in user['history']:
                    if sig['id'] == sig_id and 'result' not in sig:
                        if action == "win":
                            ...
                            user['losses'] += 1
                            user['loss_streak'] += 1
                            if user['loss_streak'] == 1: # Start timer on first loss
