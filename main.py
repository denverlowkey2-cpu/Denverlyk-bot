...
CHANNEL_ID = os.getenv('CHANNEL_ID', '') # Set this in Railway vars
...
def cmd_channel(message):
    ...
    try:
        parts = message.text.split()
        if len(parts)!= 2:
            bot.reply_to(message, f"Current Channel: `{CHANNEL_ID}`\n\nUsage: /channel <channel_id>\nExample: /channel -1001234567890", parse_mode='Markdown')
            return
        global CHANNEL_ID
        CHANNEL_ID = parts[1]
        SETTINGS['channel_id'] = CHANNEL_ID
        save_all()
        bot.reply_to(message, f"✅ Channel set to `{CHANNEL_ID}`", parse_mode='Markdown')
...
def cmd_post(message):
    if not is_admin(message.from_user.id):
        return
    try:
        # Post a demo signal to channel for testing
        pair = random.choice(PAIRS_OTC[:3])
        ...
        if sig:
            ...
            if send_to_channel(txt):
                ...
                bot.reply_to(message, "❌ Failed. Check CHANNEL_ID")
        else:
