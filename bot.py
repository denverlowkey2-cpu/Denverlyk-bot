...
def ALL_CALLBACKS(c):
    try:
        ...
        if data.startswith("copy_"):
            if "mpesa" in data:
                bot.send_message(c.message.chat.id, f"📋 M-PESA {MPESA_NUMBER} Copied! Send code")
            else:
                bot.send_message(c.message.chat.id, f"📋 USDT {USDT_TRC20} Copied! Send TxID")
            return
        if data.startswith("have_code_"):
            days=int(data.split("_")[-1])
            bot.send_message(c.message.chat.id, f"📲 Send M-PESA code for {days} days")
            return
        if data.startswith("have_txid_"):
            days=int(data.split("_")[-1])
            bot.send_message(c.message.chat.id, f"💵 Send USDT TxID for {days} days")
            return
...
def trade_settler():
    print("Settler V22.8.15 NEON ONLY TRUE WR + BUTTONS + WINLOSS RETRY")
    while True:
        try:
            time.sleep(20)
            conn=get_db(); cur=conn.cursor()
...
print("DENVERLYK V22.8.14 NEON ONLY ALL FIXED RUNNING")
try:
    bot.remove_webhook()
    time.sleep(1)
