...
def get_daily_limit(user_id):
    tier = get_user_tier(int(user_id))
    return TIERS_CONFIG[tier]['daily_limit'] # FIXED
...
*Pair:* {escape_markdown(signal['pair'])} {mode_emoji}
...
async def log_signal_sent(pair, direction, confidence, entry_time, df_1m, confluence, user_id):
    ...
    try:
        if chart_path and tier_cfg['charts'] and is_elite_plus: # FIXED
            with open(chart_path, 'rb') as photo:
                bot.send_photo(user_id, photo, caption=f"📊 *{escape_markdown(pair)} A+ Setup*\n\nConfluence: {confidence}%\nEntry: {entry_time.strftime('%H:%M:%S')}", parse_mode='Markdown')
    ...
    try:
        bot.send_message(
            ADMIN_ID,
            text=f"📊 Result check: {escape_markdown(pair)} {direction} from {entry_time.strftime('%H:%M')}\n\nDid it win?\n/win_{signal_id}\n/loss_{signal_id}",
            parse_mode='Markdown'
        )
    except Exception as e:
...
def check_expired_vips():
    ...
    for user_id in expired_users:
        try:
            ...
            username = escape_markdown(user_data.get(str(user_id), {}).get('username', 'user'))
...
*Renew via M-Pesa:*
Send to: *{escape_markdown(MPESA_NUMBER)}*
Reference: @{username}

Screenshot to {escape_markdown(SUPPORT_HANDLE)}
...
def start(message):
    ...
    if has_access(uid_str):
        ...
        bot.send_message(message.chat.id, f"🚀 *SMC ELITE BOT v3.3*\n\nPick your market to unlock A+ signals:\n\n*Pay via M-Pesa:* {escape_markdown(MPESA_NUMBER)}\n*Support:* {escape_markdown(SUPPORT_HANDLE)}", parse_mode='Markdown', reply_markup=markup)

@bot.message_handler(commands=['help'])
def cmd_help(message):
    user_id = message.from_user.id
    tier = get_user_tier(user_id)
    username = escape_markdown(message.from_user.username or message.from_user.first_name)
...
2️⃣ *Pay via M-Pesa* → {escape_markdown(MPESA_NUMBER)}
3️⃣ *Send screenshot* → {escape_markdown(SUPPORT_HANDLE)}
4️⃣ */getsignal* → Start scanning

🎁 *Try first:* /demo for free signal

❓ *Questions?* {escape_markdown(SUPPORT_HANDLE)}
"""
    else:
        mode = USERS_DATA.get(user_id, {}).get('mode', 'PO')
        help_text = f"""
...
❓ *Need help?* {escape_markdown(SUPPORT_HANDLE)}
"""
...
*Renew:* {escape_markdown(SUPPORT_HANDLE)}
""", parse_mode='Markdown')
...
*Pay via M-Pesa:* {escape_markdown(MPESA_NUMBER)}
*Send screenshot:* {escape_markdown(SUPPORT_HANDLE)}
""", parse_mode='Markdown')
...
def cmd_getsignal(message):
    ...
    tier = get_user_tier(user_id)
    tier_cfg = TIERS_CONFIG # FIXED
    pairs = PAIRS_FOREX if mode == 'FOREX' and tier_cfg['forex'] else PAIRS_OTC # FIXED
    
    markup = types.InlineKeyboardMarkup(row_width=3)
    buttons = [types.InlineKeyboardButton(p.replace('_OTC',''), callback_data=f"scan_{p}") for p in pairs[:tier_cfg['pairs']]] # FIXED
    markup.add(*buttons)
...
def cmd_myid(message):
    username = escape_markdown(message.from_user.username)
    bot.reply_to(message, f"🆔 *Your Telegram ID*\n\nUser: @{username}\nID: `{message.from_user.id}`\n\nSend this ID + M-Pesa screenshot to {escape_markdown(SUPPORT_HANDLE)}", parse_mode='Markdown')
...
def cmd_mystats(message):
    ...
    history = "\n".join([f"{escape_markdown(s['pair'])} {s['direction']} {s['confidence']}%" for s in stats['signal_history'][:5]]) or "No signals yet"
    ...
    try:
        ...
        for sig in data['signals']:
            if sig['id'] == signal_id and sig['result'] == 'pending':
                ...
                bot.reply_to(message, f"✅ Logged {result.upper()} for {escape_markdown(sig['pair'])}\nStreak: {data['streak']}")
                return
...
def callback_handler(call):
    ...
    try:
        if call.data.startswith("pay_"):
            ...
            bot.edit_message_text(f"💰 *Payment for {market}*\n\n1. Send to: *{escape_markdown(MPESA_NUMBER)}*\n2. Ref: @{escape_markdown(call.from_user.username)}\n3. Send screenshot to {escape_markdown(SUPPORT_HANDLE)}", call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=markup)
            ...
            bot.answer_callback_query(call.id, f"Scanning {escape_markdown(pair)}...")
            bot.edit_message_text(f"🔍 Scanning {escape_markdown(pair)} for A+ setup...\n\nScans today: {scans+1}/{limit}", call.message.chat.id, call.message.message_id)
            ...
            data = load_signals()
            bot.answer_callback_query(call.id, f"Wins: {data['stats']['wins']} | Losses: {data['stats']['losses']}")
