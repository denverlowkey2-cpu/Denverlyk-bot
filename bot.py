...
PAIRS = [
    'EURUSD_OTC', 'GBPUSD_OTC', 'AUDCAD_OTC',
    'XAUUSD_OTC', 'EURJPY_OTC', 'USDCAD_OTC'
]
...
price_cache = {}
CACHE_DURATION = 60 # 1min cache for Pocket Option

# ===== MESSAGES =====
START_MSG = f"""
🔥 *DENVERLYK SIGNALS - POCKET OPTION*
...
✅ 10AM & 3PM EAT auto scans
✅ Live 1min data on A+ setups
...
def get_td_data(pair, interval='1min', outputsize=200, force_fresh=False):
    ...
    # Remove _OTC for TwelveData API
...
# ===== POCKET OPTION STRATEGY FUNCTIONS =====
def check_killzone():
    now_utc = datetime.now(timezone.utc)
    hour = now_utc.hour
    return (7 <= hour < 10) or (12 <= hour < 15) # 10-12 EAT, 15-18 EAT
...
def analyze_pocket_pair(pair, is_vip, user_min_conf):
    ...
    if bos_dir!= 0: confidence += 20; direction = bos_dir
    if fvg_dir == direction and fvg_dir!= 0: confidence += 20
    if ob_dir == direction and ob_dir!= 0: confidence += 20
    if htf_trend == direction and htf_trend!= 0: confidence += 20
    if killzone: confidence += 20
...
def format_pocket_signal(signal, is_vip):
    vip_tag = "💎 *VIP A+ SIGNAL*" if signal['grade'] == 'A+' and is_vip else "📊 *B+ SIGNAL*"
...
*Entry:* Next 1m candle @ {signal['entry_time']} EAT
*Expiry:* {signal['expiry']}

*Grade:* {signal['grade']} ({signal['confidence']}/100)
*Confidence:* {signal['confidence']}%

⚠️ Enter at {signal['entry_time']} sharp. Do not late enter.
...
def auto_scan_pocket_vip():
    ...
    for pair in PAIRS:
        ...
        if signal and signal['confidence'] >= 75:
            signal_text = format_pocket_signal(signal, is_vip=True)
            for uid_str in user_data:
                uid = int(uid_str)
                is_user_vip = user_data[uid_str]['is_vip'] or uid == ADMIN_ID
                if is_user_vip and has_access(uid_str):
                    try:
                        bot.send_message(uid, f"🎯 *AUTO KILLZONE ALERT*\n\n{signal_text}", parse_mode='Markdown')
                        signals_sent += 1
                        time.sleep(0.3)
                    except Exception as e:
                        print(f"Failed to send auto ping to {uid}: {e}", flush=True)

    if signals_sent > 0:
        try:
            bot.send_message(ADMIN_ID, f"✅ Auto scan complete. Sent {signals_sent} A+ signals to VIPs.", parse_mode='Markdown')
        except: pass
...
def callback_handler(call):
    ...
    if call.data == "choose_normal":
        ...
        msg += "_Results tracking coming soon_"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="back_menu"))
        ...
        if uid == ADMIN_ID:
            ...
            btn4 = types.InlineKeyboardButton("⚙️ Settings", callback_data="settings")
            markup.add(btn1, btn2, btn3, btn4)
            expiry_str = user_data[user_id]['vip_expiry'][:10] if user_data[user_id]['is_vip'] else user_data[user_id]['normal_expiry'][:10] if user_data[user_id]['is_normal'] else "N/A"
            tier = "VIP 💎" if user_data[user_id]['is_vip'] else "Normal 💰" if user_data[user_id]['is_normal'] else "None"
            bot.edit_message_text(
                f"🔥 *Welcome back {tier}*\n\nExpires: {expiry_str}\n\nSelect an option:",
                call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=markup
            )
        ...
        if signal:
            ...
            save_data()
            signal_text = format_pocket_signal(signal, is_vip)
...
def killzone_ping():
    for uid in user_data:
        if (user_data[uid]['is_vip'] or int(uid) == ADMIN_ID) and user_data[uid]['settings']['killzone_pings'] and not is_quiet_hours(uid):
            try:
                bot.send_message(int(uid), "🎯 *Killzone Active*\n\nLondon/NY session is live. Bot scanning for A+ entries...", parse_mode='Markdown')
            except:
                pass
