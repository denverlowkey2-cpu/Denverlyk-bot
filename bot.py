...
# Initialize DB and load users on startup
init_db()
...
def sync_vip_status(user_id):
    """Sync USERS_DATA VIP status to user_data for signal checks"""
    uid = str(user_id)
...
def cmd_adduser(message):
    ...
    try:
        ...
        USERS_DATA[target_id] = {'tier': tier, 'expiry': expiry_date}
