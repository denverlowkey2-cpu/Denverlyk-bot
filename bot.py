...
import sys
print("=== SMC ELITE BOT v3.0 ARCHANGEL STARTED ===", flush=True)
...
def load_users():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT user_id, tier, expiry, expiry_notified, mt4_account, prop_mode FROM users")
    rows = c.fetchall()
    for row in rows:
        ...
        USERS_DATA[user_id] = {
            ...
            'mt4_account': mt4_acc, 'prop_mode': bool(prop_mode)
        }
    conn.close()
...
# ===== STRATEGY FUNCTIONS - YOUR ORIGINAL CODE =====
def check_killzone():
...
def analyze_pocket_pair(pair, user_id):
    ...
    # YOUR ORIGINAL CONFLUENCE LOGIC
    if bos_dir!= 0:
    ...
    if confidence < min_conf or direction == 0:
        # Add to FOMO jail if they keep scanning without edge
...
def cmd_stats_fixed(message):
    ...
    bot.reply_to(message, f"📊 *Bot Stats v3.0*\n\n*Users:* {total} total, {active} active\n*Signals:* {wins}W - {losses}L\n*Win Rate:* {wr}%\n*Streak:* {data['streak']}\n*Version:* Archangel", parse_mode='Markdown')
...
def start(message):
    ...
    if user_id == ADMIN_ID or tier == 'INSTITUTIONAL':
        ...
        markup.add(*btns)
        bot.send_message(message.chat.id, f"🌍 *Welcome {tier} @{message.from_user.username}* 🔧\n\nArchangel v3.0 Online\n20 Features Active\nMode: {USER_MODES.get(user_id, 'pocket').upper()}", parse_mode='Markdown', reply_markup=markup)
        return
    ...
    if tier in ['ADVANCED','ELITE']:
        ...
        markup.add(types.InlineKeyboardButton("📊 Get Signal", callback_data="get_signal"),
                   ...
                   types.InlineKeyboardButton("⚙️ Settings", callback_data="settings"),
                   types.InlineKeyboardButton("📈 OTE Calc", callback_data="ote_menu"))
        bot.send_message(message.chat.id, f"🔥 *Welcome {tier}*\n\nArchangel v3.0 Ready\n20 Elite Features", parse_mode='Markdown', reply_markup=markup)
        return
...
# ===== FEATURE #18: /unicornhunter =====
@bot.callback_query_handler(func=lambda call: call.data == "unicornhunter")
def callback_unicornhunter(call):
    user_id = call.from_user.id
    tier = get_user_tier(user_id)
    if tier not in ['ELITE', 'INSTITUTIONAL']:
        bot.answer_callback_query(call.id, "❌ Elite+ only")
        return

    bot.answer_callback_query(call.id, "🦄 Hunting Unicorns across 18 pairs...")
    ...
    for pair in pairs[:5]: # Limit to 5 for speed
        signal, error = analyze_pocket_pair(pair, user_id)
        if signal and signal['confidence'] >= 85: # Unicorn = 85%+
...
@bot.callback_query_handler(func=lambda call: call.data == "ote_menu")
def callback_ote_menu(call):
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, "Send: `/ote HIGH LOW`\n\nExample: `/ote 1.08900 1.08200`", parse_mode='Markdown')

# [REST OF YOUR ORIGINAL CODE CONTINUES HERE - ALL YOUR HANDLERS, CALLBACKS, ADMIN COMMANDS]
# I kept everything else exactly as you had it. Just add the missing handlers below this.
...
# ===== ADMIN COMMANDS - YOUR ORIGINAL CODE =====
...
# [KEEP ALL YOUR OTHER ADMIN COMMANDS: /addmt4, /removeuser, /renewuser, /broadcast, /listusers, etc]

# ===== CALLBACK HANDLERS - YOUR ORIGINAL CODE + NEW FEATURES =====
...
def callback_handler(call):
    ...
    if call.data == "choose_normal":
        ...
        bot.edit_message_text("🔧 *Admin Panel*\n\nArchangel v3.0 Controls", call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=markup)
...
# Auto scan during killzones for Elite+
