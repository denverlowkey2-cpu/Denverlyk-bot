...
FOMO_TEXTS=["Don't just watch! Others just profited from this signal in bot! Tap below and get yours in 3 seconds!","🔥 12 traders just took this {pair} signal - you're missing out!","⚡ {pair} pumping right now - entry closing in seconds!","💰 {pair} just hit TP! Next signal loading - tap FAST!","👀 47 users viewing {pair} right now - don't be last!","🚀 {pair} whale alert! Smart money entering NOW!","⏰ {pair} setup expires in 2 min - tap before it's gone!","🔔 {pair} ALERT: 89% win rate on this pattern last week!","💎 {pair} diamond setup - only 3% get this quality!","⚠️ {pair} breaking out - 20 people already in profit!","🎯 {pair} perfect retest - institutions buying here!","🔥 {pair} momentum building - FOMO is real!","💵 {pair} +$127 average profit today - your turn!","🏆 {pair} top performer - 6 wins in a row!","🌊 {pair} wave incoming - catch it or watch others win!"]
def get_next_fomo():
...
def get_binance_klines(symbol, interval='5min', limit=80):
    try:
        ...
        limit=min(limit,200)
        endpoints=[
            f"https://api.binance.com/api/v3/klines?symbol={bsym}&interval={tf}&limit={limit}",
            f"https://data-api.binance.vision/api/v3/klines?symbol={bsym}&interval={tf}&limit={limit}",
            f"https://api1.binance.com/api/v3/klines?symbol={bsym}&interval={tf}&limit={limit}",
        ]
...
def get_klines(s,i='5min',l=80):
    try:
        if s in BINANCE_MAP:
            ...
            if b: return b
            td=get_twelvedata_klines(s,i,l)
            if td: return td
...
def calc_pro(klines, mode="POCKET", channel_mode=False, pair=""):
    ...
    plus_dm_raw=pd.Series(np.where((up_move>down_move) & (up_move>0), up_move, 0.0))
    minus_dm_raw=pd.Series(np.where((down_move>up_move) & (down_move>0), down_move, 0.0))
    ...
    dx_series=100*abs(plus_di-minus_di)/(plus_di+minus_di).replace(0,1)
    adx=float(wilder_rma(dx_series,14).iloc[-1])
    if pd.isna(adx) or adx<10: adx=18+random.uniform(0,6)
    ema12=close_s.ewm(span=12).mean(); ema26=close_s.ewm(span=26).mean()
    macd_line=ema12-ema26; signal_line=macd_line.ewm(span=9).mean()
    macd_bull=macd_line.iloc[-1] > signal_line.iloc[-1]
    vol_avg=np.mean(vols[-20:]) if len(vols)>=20 else np.mean(vols); vol_now=vols[-1]
    vol_ok = vol_now > vol_avg*1.1 if vol_avg>0 else True
    session=get_session(); ema_gap=abs(ema21-ema50)/closes[-1]*100
    price=closes[-1]; atr_p=atr/price*100; dist_ema=abs(price-ema21)/price*100
    ...
    min_adx = 11 if is_forex else MIN_ADX_STRICT
    min_atr_p = 0.01 if is_forex else MIN_ATR_P
    atr_max = 2.0 if is_forex else 2.0
    if atr_p > atr_max:
        if pair: NEWS_PAIRS_BLOCK[pair]=time.time()+900
        return None
    if channel_mode:
        if adx<min_adx: return None
        if ema_gap<0.015 and not is_forex: return None
        if rsi<32 or rsi>78: return None
        if atr_p<min_atr_p: return None
        if dist_ema>MAX_SPREAD_P and not is_forex: return None
        if not vol_ok and not is_forex: return None
    else:
        if adx<(11 if is_forex else 18): return None
...
def save_balance_handler(m):
    try:
        ...
        if any(x in txt.upper() for x in ["CRYPTO","FOREX","INDEX","BALANCE","MAIN MENU","MARKET","SUBSCRIBE","BEST","STATS","REFERRAL","RISK","POCKET","MT5","SUPPORT","1MIN","5MIN","15MIN","1HOUR","4HOUR","/USD","BTC","ETH","XAU","US30","EUR"]):
            ...
            pair = next((p for p in ALL_PAIRS if p in m.text), None)
            if pair:
                with USER_LOCK: USER_PAIR[uid]=pair
        ...
        bal=float(txt)
        if bal < 1 or bal > 1000000: bot.send_message(m.chat.id, "❌ Send 1 to 1000000 e.g. 50"); return
        save_user_balance(uid, bal); USER_AWAITING_BALANCE.pop(uid,None)
...
def check_mtf(pair, tf, direction):
    higher={"5min":"15min","15min":"1h","1h":"4h","4h":"1h","1m":"5m"}.get(tf)
...
def build_custom_vn_text(sig):
    ...
    except: return f"{sig.get('pair','BTC/USD')} {sig.get('direction','BUY')} entry {sig.get('entry',0)}"
def send_manager_vn_to_channel(sig):
    try:
        vn_text=build_custom_vn_text(sig); mp3_path=f"/tmp/vn_{sig.get('pair','BTC').replace('/','')}_{int(time.time())}.mp3"
    ...
    except Exception as e: print(f"Channel VN fail {e}")
def post_to_channel(sig, pair, tf):
    try:
        chart=generate_chart_pro(pair, sig, None)
        caption=f"🦁 {BRAND_NAME}\n{get_pair_label(pair)} {pair} {tf.upper()} {sig['direction']} {sig['strength']}\n📊 ADX {sig['adx']:.0f} RSI {sig['rsi']:.0f} Conf {sig['conf']}/5\n💰 Entry {sig['entry']:.5f} SL {sig['sl']:.5f} TP {sig['tp']:.5f}\nRR 1:{sig['rr']} Session {sig['session']}\n🔗 {BOT_LINK}"
        if chart: bot.send_photo(CHANNEL_ID, chart, caption=caption)
        else: bot.send_message(CHANNEL_ID, caption)
    except Exception as e: print(f"post channel err {e}")
def send_signal_pro(uid, pair, tf):
    try:
        with USER_LOCK: USER_TF[uid]=tf
        if is_high_volatility_block(pair):
            bot.send_message(uid, f"⏳ {pair} cooling down after recent loss/volatility. Try {pair} in 30min or another pair."); return
        mode=USER_MODE.get(uid,"POCKET")
        klines=get_klines(pair, tf, 80)
        if not klines:
            bot.send_message(uid, f"❌ No data for {pair} {tf.upper()} - try again in 10s. API limit."); return
        sig=calc_pro(klines, mode=mode, channel_mode=False, pair=pair)
        if not sig:
            bot.send_message(uid, f"📉 No setup for {pair} {tf.upper()} now. Market choppy. Try 15MIN or another pair."); return
        sig["pair"]=pair; sig["tf"]=tf
        mtf_ok, mtf_msg, mtf_adx = check_mtf(pair, tf, sig["direction"])
        chart=generate_chart_pro(pair, sig, uid)
        entry=sig["entry"]; sl=sig["sl"]; tp=sig["tp"]; tp2=sig.get("tp2")
        stake=get_user_balance(uid)
        if stake<1: stake=2.0
        fixed_profit=stake*1.9; real_profit=stake*2.5
        fomo=get_next_fomo().format(pair=pair)
        mtf_icon="✅" if mtf_ok else "⚠️"
        text=f"🦁 {BRAND_NAME} {TF_LABELS.get(tf,tf.upper())}\n{get_pair_label(pair)} {pair} {sig['direction']} {sig['strength']} | Conf {sig['conf']}/5\n{mtf_icon} MTF: {mtf_msg}\n📊 ADX {sig['adx']:.0f} RSI {sig['rsi']:.0f} {sig['session']}\n💰 Entry {entry:.5f}\n🛑 SL {sl:.5f} (-{sig['sl_p']:.2f}%)\n🎯 TP {tp:.5f} (+{sig['tp_p']:.2f}%)"
        if tp2: text+=f"\n🎯 TP2 {tp2:.5f} (+{sig['tp2_p']:.2f}%)"
        text+=f"\nRR 1:{sig['rr']} | Stake ${stake:.2f}\n💵 Fixed +${fixed_profit:.2f} Real +${real_profit:.2f}\n\n⚡ {fomo}\n⏰ Expires in {tf.upper()}"
        markup=types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton(f"📈 Take {sig['direction']} {pair}", callback_data=f"take_{pair}_{tf}_{sig['direction']}"))
        markup.add(types.InlineKeyboardButton("🔄 Refresh", callback_data=f"refresh_{pair}_{tf}"))
        if chart: bot.send_photo(uid, chart, caption=text, reply_markup=markup)
        else: bot.send_message(uid, text, reply_markup=markup)
        if sig["conf"]>=4:
            try: post_to_channel(sig, pair, tf); send_manager_vn_to_channel(sig)
            except: pass
    except Exception as e:
        print(f"send_signal_pro err {e}")
        try: bot.send_message(uid, f"❌ Error generating signal {pair} {tf}: {e}")
        except: pass
def scan_one_job(args):
    ...
    try:
        ...
        if is_forex:
            if sig["adx"]<11 or sig["conf"]<2: return None
        else:
...
def run_market_scan_thread(uid, tf):
    def scan_job():
        try:
            ...
            if not good:
                bot.send_message(uid, f"🌊 {mode} No high-quality on {actual_tf.upper()} now", reply_markup=main_menu(uid)); return
...
@bot.message_handler(func=lambda m: m.text and "Market" in m.text)
def market_cmd(m):
    if not is_active(m.from_user.id): pay_cmd(m); return
    run_market_scan_thread(m.from_user.id, "5min")
@bot.message_handler(func=lambda m: m.text and "Best Setup" in m.text)
def best_setup_cmd(m):
    if not is_active(m.from_user.id): pay_cmd(m); return
...
def risk_calc_menu(m):
    ...
    except Exception as e: print(f"risk calc err {e}")
...
def start_cmd(m):
    ...
    if is_admin(uid):
        bot.send_message(m.chat.id, f"👑 Welcome Admin {BRAND_NAME} V22.8.15 FINAL\nAll bugs fixed ✅\nClick Admin Panel", reply_markup=main_menu(uid))
    else:
        if is_active(uid):
...
def admin_cmd(m):
    ...
    bot.send_message(m.chat.id, f"👑 Admin {BRAND_NAME} V22.8.15 FINAL", reply_markup=kb)
...
def ALL_CALLBACKS(c):
    try:
        ...
        if data.startswith("risk_") or data.startswith("bal_"):
            ...
            except Exception as e: print(f"calc cb err {e}")
            return
        if data=="adm_add":
        ...
        if data=="pay_intl":
            kb=types.InlineKeyboardMarkup(row_width=2)
            kb.add(types.InlineKeyboardButton("📅 7 DAYS $16", callback_data="intl_7"), types.InlineKeyboardButton("📅 30 DAYS $50 [BEST]", callback_data="intl_30"))
            bot.send_message(c.message.chat.id, "🌍 INTERNATIONAL - Choose plan", reply_markup=kb)
            return
        if data.startswith("intl_"):
            days = 7 if "7" in data else 30
            price = "$16" if days==7 else "$50"
            kb2=types.InlineKeyboardMarkup()
            kb2.add(types.InlineKeyboardButton("📋 COPY TRC20", callback_data=f"copy_usdt_{days}"), types.InlineKeyboardButton("✅ I HAVE TxID", callback_data=f"have_txid_{days}"))
            bot.send_message(c.message.chat.id, f"💵 USDT PAYMENT {price} for {days} days\n\nTRC20: {USDT_TRC20}\n\nAfter pay tap I HAVE TxID", reply_markup=kb2)
            return
        if data.startswith("copy_"):
            bot.send_message(c.message.chat.id, "📋 Address copied! Now send TxID / M-Pesa code here to verify")
            return
        if data.startswith("reply_ticket_"):
            try:
                target=int(data.split("_")[-1])
                ADMIN_STATE[c.message.chat.id]=f"await_reply_{target}"
                bot.send_message(c.message.chat.id, f"✏️ Send reply to user {target}:")
            except: pass
            return
        if data in ["pay_kenya","back_pay"]:
        ...
        if data.startswith("admin_"):
            ...
            if data=="admin_stats":
                ...
                conn.close()
                bot.send_message(c.message.chat.id, f"⚡ FORCED {settled} trades settled!")
        ...
        if data.startswith("sup_"):
            ...
            elif data=="sup_signals": bot.send_message(uid, "📡 SIGNALS: Tap pair then TF 1MIN/5MIN/15MIN/1H/4H", reply_markup=main_menu(uid))
            elif data=="sup_loss": bot.send_message(uid, "📉 LOSSES NORMAL WR 53% RR 1:1.9 = profitable", reply_markup=main_menu(uid))
...
def catch_all(m):
    if m.text and "Market" in m.text:
        ...
        try:
            ...
            if r: w,l,fixed,real=r; tot=w+l; wr=int(w/tot*100) if tot else 0; bot.send_message(m.chat.id, f"📊 Stats {w}W {l}L WR {wr}%\n💰 Fixed: ${fixed:.2f}\n💹 Real: ${real:.2f}", reply_markup=main_menu(m.from_user.id))
            else: bot.send_message(m.chat.id, "No stats yet", reply_markup=main_menu(m.from_user.id))
...
def get_live_price(pair):
    try:
        if pair in BINANCE_MAP:
            try:
                bsym=BINANCE_MAP[pair]
                r=requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={bsym}", timeout=3).json()
                if "price" in r:
    ...
    try:
        k=get_key()
        if k:
            r=requests.get(f"https://api.twelvedata.com/price?symbol={pair}&apikey={k}", timeout=3).json()
            if "price" in r:
                return float(r["price"])
    except: pass
    return None
...
def trade_settler():
    print("Settler V22.8.15 FINAL - WIN/LOSS + LIVE DEBUG")
    while True:
        try:
            time.sleep(15)
            conn=get_db(); cur=conn.cursor()
            ...
            if ready>0:
                print(f"[SETTLER] {ready} trades ready to settle")
            ...
            for tid, uid, pair, direction, entry, expiry, tf, stake in rows:
                try:
                    ...
                    if not live:
                        print(f"[SETTLER] FAIL live price {pair} skip")
                        continue
...
print("DENVERLYK V22.8.15 ALL FIXED RUNNING 1254 LINES")
try:
    bot.remove_webhook()
    time.sleep(1)
