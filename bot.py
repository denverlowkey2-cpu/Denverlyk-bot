...
def safe_edit(chat_id, message_id, text, **kwargs):
    """Safely edit message. If edit fails, delete and send new one."""
    try:
...
def fetch_candles(pair, interval, count=100):
    ...
    try:
        ...
        params = {'symbol': pair, 'interval': interval, 'outputsize': count, 'apikey': TWELVE_API_KEY}
...
def scan_callback(call):
    ...
    # FIXED: Safe edit - no reply_markup on edit
    safe_edit(call.message.chat.id, call.message.message_id, f"🔍 Scanning {pair}...\n\nUsing 2 API calls")

    news_blocked, reason = check_news_block()
    if news_blocked:
        # FIXED: Delete old msg and send new one with buttons
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except:
            pass
        bot.send_message(call.message.chat.id, f"📰 {reason}\n\nTrading paused.", reply_markup=get_client_menu(user_id))
        return
    ...
    if pair == 'auto':
        for p in PAIRS:
            ...
            if signal:
                ...
                try:
                    ...
                    pass
                bot.send_message(call.message.chat.id, f"✅ Signal sent: {p}", reply_markup=get_client_menu(user_id))
                return
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except:
            pass
        bot.send_message(call.message.chat.id, "❌ No setup found on any pair.\n\nTry again in 1min", reply_markup=get_client_menu(user_id))
    else:
        ...
        if signal:
            ...
            try:
                ...
                pass
            bot.send_message(call.message.chat.id, f"✅ Signal sent: {pair}", reply_markup=get_client_menu(user_id))
        else:
            # FIXED: Delete old msg and send new one with buttons - THIS WAS LINE 382
            try:
                bot.delete_message(call.message.chat.id, call.message.message_id)
            except:
                pass
            bot.send_message(call.message.chat.id, f"❌ No setup for {pair}.\n\nTry another pair or wait 1min", reply_markup=get_client_menu(user_id))
