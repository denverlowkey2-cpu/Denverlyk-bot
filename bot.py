...
def calc_pro(klines, mode="POCKET", channel_mode=False, pair=""):
    ...
    min_atr_p = 0.02 if is_forex else MIN_ATR_P
...
def send_signal_pro(uid, pair, tf):
    ...
    if not klines:
        bot.send_message(uid, f"❌ {pair} {tf.upper()} data cooling, try 1m later", reply_markup=main_menu(uid))
        return
...
@bot.message_handler(func=lambda m: ADMIN_STATE.get(m.chat.id) in ["await_add_id","await_remove_id","await_reply_id"] or (isinstance(ADMIN_STATE.get(m.chat.id), str) and ADMIN_STATE.get(m.chat.id).startswith("await_reply_")))
def admin_id_input(m):
    try:
        ...
        if state and state.startswith("await_reply_"):
...
print("DENVERLYK V22.8.15 ALL FIXED RUNNING")
try:
    bot.remove_webhook()
    time.sleep(1)
