...
def start(message):
    ...
    # 1. VIP CHECK FIRST - even for admin
    if user_id in USERS_DATA:
...
def callback_handler(call):
    ...
    uid = int(user_id) # Use caller's ID, not message.from_user.id
    ...
    if call.data == "choose_normal":
        ...
        is_vip = user_data[user_id]['is_vip'] or uid == ADMIN_ID # ADMIN COUNTS AS VIP
    ...
    # Admin panel callbacks - FIXED TO USE CORRECT USER ID
        ...
        cmd_listusers(FakeMsg(uid, call.message.chat.id))
        ...
        class FakeMsg:
            def __init__(self, uid, chat_id):
                self.from_user = type('User', (), {'id': uid})()
                self.chat = type('Chat', (), {'id': chat_id})()
        cmd_clearexpired(FakeMsg(uid, call.message.chat.id))
        ...
        class FakeMsg:
            def __init__(self, uid, chat_id):
                self.from_user = type('User', (), {'id': uid})()
                self.chat = type('Chat', (), {'id': chat_id})()
        cmd_stats(FakeMsg(uid, call.message.chat.id))
